...
class Companion {
    companion object {
        ...
        fun readCard(posicion:Int):List<*>{
            ...
            return listOf(card!!.nombre,card!!.id,card!!.level,card!!.graduado,card!!.precio)
        }

        fun readExpansion(posicion:Int):List<*>{
            val name = expansiones.keys.toList()[posicion]
            val exp= expansiones.get(name)
            return listOf(exp!!.nombre,exp!!.id,exp!!.releaseDate,exp!!.precio,exp!!.activa, exp!!.cartas)
        }
