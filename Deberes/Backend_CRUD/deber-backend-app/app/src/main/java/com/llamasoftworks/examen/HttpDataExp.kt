...
class HttpDataExp {
    ...
    fun readExpansionNames(){
        val (request, response, result) = "http://192.168.100.29:1337/materia"
            .httpGet()
            .responseString()
        when(result){
            is Result.Success -> {
    ...
    fun readExpansion(posicion:Int):List<*>{
        ...
        val url = urlPrincipal + "/materia?nombre="+nombre
            ...
            is Result.Success -> {
                ...
                if (expansion != null){
                    ...
                    listaDeDatosExpansion = mutableListOf(expansion[0].nombre,expansion[0].id,
                        expansion[0].releaseDate,expansion[0].precio,expansion[0].activa,expansion[0].cartas)
                }
            }
            is Result.Failure -> {
                val ex = result.getException()
    ...
    fun updateExpansion(newName: String, id: String,
                        releaseDate: LocalDate, activa: Boolean, precio: Double,
                        listCartas: ArrayList<String>){
        val url = urlPrincipal + "/materia/"+id
    ...
    fun deleteExpansion (idExpansion:String){
        val url = urlPrincipal + "/materia/"+idExpansion
        url.httpDelete()
            .responseString{
                    req, res, result ->
                when(result){
